...
import { User, Phone, Shield, ShieldCheck, Mail, Building, Save, AlertCircle } from 'lucide-react';
...
export default function WardenProfile() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      console.error('Failed to update warden profile:', err);
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <PageHeader
        title="Warden Profile Settings"
        subtitle="Manage your contact details, view hostel assignments, and update your console access password."
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-850 p-6 rounded-2xl shadow-xs">
            <form onSubmit={handleUpdateProfile} className="space-y-6 text-xs">
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-850">
                <Button type="submit" icon={Save} loading={loading}>Save Profile Details</Button>
              </div>
            </form>
          </div>
        ...
        <div className="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-850 p-6 rounded-2xl shadow-xs h-fit text-xs space-y-4">
          <h3 className="font-extrabold text-sm border-b border-slate-100 dark:border-slate-850 pb-2 flex items-center gap-2">
            ...
            Assigned PG Hostel Details
          </h3>

          <div className="space-y-3 font-semibold text-slate-655 dark:text-slate-350">
            <div>
              <span className="text-[10px] text-slate-400 block font-normal">Assigned PG Hostel</span>
              <p className="text-slate-900 dark:text-white font-bold">{profile.hostelName || 'All PG Hostels'}</p>
            </div>
            <div>
              <span className="text-[10px] text-slate-400 block font-normal">Console Access Role</span>
              <p className="text-slate-900 dark:text-white font-bold uppercase tracking-wider text-[10px]">{profile.role}</p>
            </div>
          </div>
        </div>
      </div>
